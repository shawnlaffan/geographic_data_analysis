...
If you are using a shapefile then set ```source_db``` to be ```source_dir```.  If a geodatabase then specify the geodatabase directory name.  If a geopackage then use the .gpkg file name.

Directories are called "folders" on windows.

Note that rgdal can read geodatabase feature classes and tables, but cannot wite to them. We will use the geopackage format for oututs by default as they can be read by both QGIS and ArcGIS.
...
writeOGR(output, output_db, output_layer_name, driver=output_driver)
