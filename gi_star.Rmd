...
If you are using a shapefile then specify the directory containing the file a the ```source_db```.  If a geodatabase then specify the geodatabase directory.  If a geopackage then use the path to the gpkg file.
...
output_db  = paste(output_dir, "experiment.gpkg", sep="/")
...
Now we need to save the data to a file.  We will use the geopackage format for the spatial data.
This will fail if you have the output open in a GIS.

```{r}

writeOGR(output, output_db, output_layer_name, driver="GPKG")
