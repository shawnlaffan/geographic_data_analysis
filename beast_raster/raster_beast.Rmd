...
list.of.packages <- c("terra", "Rbeast", "stringi", "svDialogs", "ncdf4", "lubridate")
...
#    period = 32/365
  ...
  o$ext = as.vector(ext(raster)) #  ext objects do not survive serialisation
  o$crs = crs(raster)
...
plot_beast_modis_coord = function (b, coord, t=FALSE) {
  ...
  message ("Plotting image row ", rowcol[1], ", col ", rowcol[2])
...
overwrite_flag = svDialogs::dlg_list  (choices = c("No", "Yes"), title = "Overwrite existing files?")$res
overwrite_flag = ifelse (length(save_flag) > 0 && save_flag == "Yes", TRUE, FALSE)

export_beast_rasters(beast_result, output_dir, output_prefix, overwrite_flag)
